...
int main(int argc, char* argv[])
{
	...
	file_buffer = (char*)malloc(file_stat.st_size+12);
	if(NULL == file_buffer)
	{
	...
	*((unsigned long*)file_buffer+1) = file_stat.st_size+10;	//file size
	...
	printf("Writing data...\n");
	size_t remain_size = file_stat.st_size+12;
	...
	while(remain_size>0)
	{
		...
		writed += to_write;
		printf("\r%d%\t %d bytes     ", ((unsigned long)(writed*100))/(file_stat.st_size+10), writed);
		fflush(stdout);

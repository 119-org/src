...
#define REGISTER_AGENT(x) { \
    extern struct agent ipc_##x##_agent; \
    agent_register(&ipc_##x##_agent, sizeof(ipc_##x##_agent)); }
...
int agent_register_all()
{
    ...
    REGISTER_AGENT(videocap);
    REGISTER_AGENT(x264enc);
    REGISTER_AGENT(display);
    REGISTER_AGENT(upstream);
...
static void on_agent_read(int fd, short what, void *arg)
{
    ...
    if (ctx->yq_src) {
        ...
        //printf("ctx = %p yqueue_pop %p, last=%d\n", ctx, in_item, last);
    ...
    if (ctx->yq_src) {
        //printf("ctx = %p yqueue_item_free %p, last=%d\n", ctx, in_item, last);
        if (last) {
            usleep(100000);//FIXME

...
int sock_get_host_list(char ip_list[][64], int list_size)
{
	...
	for (temp_if = interfaces, i = 0; temp_if; temp_if = temp_if->ifa_next, i++) {
		...
		if (temp_if->ifa_addr != NULL) {
			...
			switch (temp_if->ifa_addr->sa_family) {
				case AF_INET:
					...
					inet_ntop(AF_INET, (void *)&(ipv4->sin_addr), ip_str, sizeof(ip_str));
					break;
				case AF_INET6:
					ipv6 = (struct sockaddr_in6 *)temp_if->ifa_addr;
					inet_ntop(AF_INET6, (void *)&(ipv6->sin6_addr), ip_str, sizeof(ip_str));
					break;
				default:
					break;
			}
...
int sock_get_host_list_byname(char ip_list[][64], const char *byname, int list_size)
{
	struct hostent* host;
	char str[64] = {0};
	char **p;

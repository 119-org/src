...
/* this structure is retreived by the core application
   with dlsym */
psp_plugin_t PSP_PLUGIN_DECLARE_DATA ls_sfull_plugin;
...
sfp_plugin_t *ls_sfull_plug;
char sfull_name_config[50];

extern ls_sfull_ctx_t *ls_sfull_context;

/* this is called by the core application when the module
   is loaded (the address of this method is located in
   the structure ls_sfull_plugin->plugin_init() */
...
plugin_init (char *name_config)
{
  ...
  OSIP_TRACE (osip_trace (__FILE__, __LINE__, OSIP_INFO2, NULL,
			  "ls_sfull plugin: plugin_init()!\n"));


#ifndef WIN32
  if (name_config==NULL)
    snprintf(sfull_name_config, 49, "sfull");
  else
    snprintf(sfull_name_config, 49, name_config);
#else
  if (name_config==NULL)
    _snprintf(sfull_name_config, 49, "sfull");
  else
    _snprintf(sfull_name_config, 49, name_config);
#endif
  ...
  psp_plugin_take_ownership (&ls_sfull_plugin);
  i = psp_core_load_sfp_plugin (&ls_sfull_plug, &ls_sfull_plugin);
  if (i != 0)
    goto pi_error2;

  /* INIT HOOK METHOD */
  i =
    sfp_inc_func_init (&fn, &cb_ls_sfull_search_location,
		   ls_sfull_plugin.plug_id);
  if (i != 0)
    goto pi_error3;
...
{
  OSIP_TRACE (osip_trace (__FILE__, __LINE__, OSIP_INFO2, NULL,
			  "ls_sfull plugin: plugin_release()!\n"));
  ls_sfull_ctx_free ();
...
psp_plugin_t PSP_PLUGIN_DECLARE_DATA ls_sfull_plugin = {
  0,				/* uninitialized */
  "Stateless handling plugin",
  "0.5.1",
  "plugin which mandates some transactions to be handled statelessly",
  PLUGIN_SFP,
